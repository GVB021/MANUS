...
export class DatabaseStorage implements IStorage {
  ...
  async deleteUser(id: string): Promise<void> {
    // Primeiro deletar todas as dependências do usuário para evitar erro de Foreign Key
    ...
    // Atualizar takes e personagens para nulo em vez de deletar para manter histórico se necessário, 
    // ou deletar se preferir. Aqui vamos apenas desvincular para evitar erros.
    await db.update(characters).set({ voiceActorId: null }).where(eq(characters.voiceActorId, id));
    await db.update(sessions).set({ createdBy: null }).where(eq(sessions.createdBy, id));
  ...
  async deleteStudio(id: string): Promise<void> {
    // Cascata manual para deletar estúdio
    const memberships = await db.select({ id: studioMemberships.id }).from(studioMemberships).where(eq(studioMemberships.studioId, id));
    if (memberships.length > 0) {
      const membershipIds = memberships.map(m => m.id);
      await db.delete(userStudioRoles).where(inArray(userStudioRoles.membershipId, membershipIds));
      await db.delete(studioMemberships).where(eq(studioMemberships.studioId, id));
    }
    await db.delete(studioProfiles).where(eq(studioProfiles.studioId, id));
    await db.delete(staff).where(eq(staff.studioId, id));
    
    // Deletar sessões (que deletam takes e participantes)
    const studioSessions = await db.select({ id: sessions.id }).from(sessions).where(eq(sessions.studioId, id));
    for (const s of studioSessions) {
      await this.deleteSession(s.id);
    }
    
    // Deletar produções (que deletam personagens)
    const prods = await db.select({ id: productions.id }).from(productions).where(eq(productions.studioId, id));
    for (const p of prods) {
      await this.deleteProduction(p.id);
    }
  ...
  async deleteProduction(id: string): Promise<void> {
    // Cascata manual para deletar produção
    const prodsCharacters = await db.select({ id: characters.id }).from(characters).where(eq(characters.productionId, id));
    if (prodsCharacters.length > 0) {
      const charIds = prodsCharacters.map(c => c.id);
      await db.delete(takes).where(inArray(takes.characterId, charIds));
      await db.delete(characters).where(eq(characters.productionId, id));
    }
    
    // Deletar sessões vinculadas
    const prodsSessions = await db.select({ id: sessions.id }).from(sessions).where(eq(sessions.productionId, id));
    for (const s of prodsSessions) {
      await this.deleteSession(s.id);
    }
  ...
  async deleteSession(id: string): Promise<void> {
    // Cascata manual para deletar sessão
